...
#resource "null_resource" "send-sms-on-completion" {
#  triggers = {
#    jenkins_url = join(",", [aws_instance.jenkins-master])
#  }
#  provisioner "local-exec" {
#    command = "aws --profile ${var.profile} sns publish --message 'Jenkins Deployment completed' --subject 'Terraform' --phone-number '${var.phone_number}'"
#  }
#}
