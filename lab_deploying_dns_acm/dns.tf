...
resource "aws_route53_record" "cert_validation" {
  provider = aws.region-master
  name     = aws_acm_certificate.aws-ssl-cert.domain_validation_options.0.resource_record_name
  type     = aws_acm_certificate.aws-ssl-cert.domain_validation_options.0.resource_record_type
  zone_id  = data.aws_route53_zone.dns.zone_id
  records  = [aws_acm_certificate.aws-ssl-cert.domain_validation_options.0.resource_record_value]
  ttl      = 60
}
